(function () {
  ...
  const downloadTrack = (meta) => {
    ...
    const infoUrl = 'https://storage.mds.yandex.net/download-info/' + storageDir + '/2'
  ...
  const downloadHqTrack = (meta) => {
    ...
    const albumId = meta.track.albumId || meta.track.albums && meta.track.albums[0] && meta.track.albums[0].id;
    ...
    proxy(function (trackId) {
      ...
      const metaUrl = 'https://music.yandex.ru/api/v2.1/handlers/track/' + trackId + '/web-artist_tracks-track-track-main/download/m?hq=1&external-domain=music.yandex.ru&overembed=no&__t=' + timestamp
      const data = {
        "timestamp": timestamp,
      ...
      request.setRequestHeader('X-Retpath-Y', encodeURIComponent(document.location.href))
      request.setRequestHeader('X-Current-UID', ya.music.user.getCurrentUID())
      request.setRequestHeader('Accept', 'application/json; q=1.0, text/*; q=0.8, */*; q=0.1')
      request.setRequestHeader('X-Requested-With', 'XMLHttpRequest')
      request.onerror = (e) => {
        returnError('Ya.Music Helper: HQ Info request has triggered on error: ' + e.target.status)
      }
      request.onreadystatechange = () => {
        if (request.readyState !== XMLHttpRequest.DONE) {
          return
        ...
        if (request.status === 200 &&
          ...
          returnError('Ya.Music Helper: Status code is not 200 during resolving HQ info data or SRC is not in response')
        }
      }
      request.send()
    }, trackId)
